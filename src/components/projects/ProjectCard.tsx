...
export const ProjectCard = ({project, isDarkMode, language, onClick}: ProjectCardProps) => {
                ...
                <div className="flex flex-wrap gap-2">
                    {
                        project.tech.map((tech) => (
                            <span key={tech} className={`px-3 py-1 text-sm rounded-full transition-colors duration-200 ${isDarkMode ? 'bg-gray-600 hover:bg-gray-500' : 'bg-gray-100 hover:bg-gray-200'}`}>
                                {tech}
                            </span>
                        ))
                    }
