...
export function ProjectModal({project, isOpen, onClose, isDarkMode, language}: Props) {
    ...
    type LocalizedField = 'title' | 'description' | 'features' | 'duration' | 'team' | 'role' | 'challenges';
    const getLocalizedField = (field: LocalizedField) => project[`${field}_${language}` as keyof Project] || project[`${field}_en` as keyof Project];
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div className={`relative w-full max-w-4xl max-h-[90vh] overflow-y-auto rounded-2xl shadow-2xl ${
                ...
                {/* Header */}
                <div
                    className="sticky top-0 z-10 flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700">
                ...
                <div className="p-6 space-y-6">
                    ...
                    <div className="relative overflow-hidden rounded-xl">
                        <img src={project.image} alt={title} className="w-full h-64 object-cover"/>
                    ...
                    <div className="grid md:grid-cols-2 gap-6">
                        ...
                        <div className="space-y-6">
                            ...
                            {/* Technologies */}
                            <div>
                                <h3 className="text-lg font-semibold mb-3">{currentT.technologies}</h3>
                                <div className="flex flex-wrap gap-2">
                                    {
                                        project.tech.map((tech) => (
                                        <span key={tech} className={`px-3 py-2 text-sm rounded-lg font-medium transition-colors ${isDarkMode ? 'bg-gray-700 text-gray-200 hover:bg-gray-600' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
                                            {tech}
                                        </span>
                                        ))
                                    }
                            ...
                            {/* Features */}
                            {
                                features && features.length > 0 && (
                                    <div>
                                        <h3 className="text-lg font-semibold mb-3">{currentT.features}</h3>
                        ...
                        <div className="space-y-6">
                            ...
                            <div className={`p-4 rounded-lg ${isDarkMode ? 'bg-gray-700/50' : 'bg-gray-50'}`}>
                                <h3 className="text-lg font-semibold mb-3">{currentT.projectDetails}</h3>
                            ...
                            {/* Challenges & Solutions */}
                            {
                                challenges && (
                                <div>
                                    <h3 className="text-lg font-semibold mb-3">{currentT.challenges}</h3>
                                    <div className={`p-4 rounded-lg ${isDarkMode ? 'bg-gray-700/50' : 'bg-gray-50'}`}>
