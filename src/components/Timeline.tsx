...
export function Timeline({ isDarkMode, title, subtitle, items }: Props) {
  return (
    <section
      ...
      className={`min-h-screen py-20 ${isDarkMode ? 'bg-gray-900/60' : 'bg-gray-100/60'}`}
    >
          ...
          <div className='space-y-6'>
            {items.map((item, index) => (
              <div key={index} className='relative'>
                ...
                {/* Mobile timeline dot */}
                <div
                  className={`absolute left-6 top-6 w-3 h-3 rounded-full border-2 transform -translate-x-1/2 ${item.type === 'education' ? 'bg-blue-400 border-blue-200' : 'bg-purple-400 border-purple-200'} z-10`}
                ></div>

                {/* Mobile content */}
                <div className='ml-12'>
                  <div
                    className={`p-4 rounded-lg border transition-all duration-300 ${isDarkMode ? 'bg-gray-800/50 border-gray-700' : 'bg-white border-gray-200 shadow-sm'}`}
                  >
                    <div className='flex items-center mb-3'>
                      {item.type === 'education' ? (
                        <GraduationCap
                      ...
                      <span
                        className={`text-xs font-semibold px-2 py-1 rounded-full ${item.type === 'education' ? 'bg-blue-100 text-blue-800' : 'bg-purple-100 text-purple-800'}`}
                      >
                        {item.year}
                      </span>
                    </div>
                    ...
                    <div className='flex items-center mb-2'>
                      ...
                      <MapPin
                        size={14}
                        className={`ml-1 mr-1 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}
                      />
                      <span
