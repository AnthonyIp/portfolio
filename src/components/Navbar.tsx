...
type Props = {
    ...
    labels: { home: string; about: string; timeline: string; projects: string; contact: string };
};

export function Navbar({isDarkMode, setIsDarkMode, language, setLanguage, labels}: Props) {
    ...
    return (
        <nav className={`fixed w-full z-50 transition-all duration-300 ${scrollY > 50 ? (isDarkMode ? 'bg-gray-900/95 backdrop-blur-md shadow-lg' : 'bg-white/95 backdrop-blur-md shadow-lg') : 'bg-transparent'}`}>
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="flex justify-between items-center py-4">
                    ...
                    <div className="hidden md:flex space-x-8">
                        ...
                        >
                            <span className="text-xl" role="img" aria-label={language === 'en' ? 'French' : 'English'}>
                                {
                                    language === 'en' ? '🇫🇷' : '🇬🇧'
                                }
                            </span>
                ...
                </div>
                {
                    isMenuOpen && (
                        <div
                            ...
                            >
                                {
                                    isDarkMode ? <Sun size={20} className="mr-2"/> :
                            ...
                            >
                            <span
                                className="text-xl mr-2">{language === 'en' ? '🇫🇷' : '🇬🇧'}</span> {language === 'en' ? 'Français' : 'English'}
                            </button>
                            {
                                ['home', 'about', 'timeline', 'projects', 'contact'].map((id) => (
                                    <button key={id}
