        ...
        <section id="about" aria-labelledby="about-heading" className={`h-screen flex items-center ${isDarkMode ? 'bg-gray-800/60' : 'bg-white/60'}`}>
            <div className="max-w-6xl mx-auto px-4 w-full">
                ...
                <div className="grid md:grid-cols-2 gap-12 items-center">
                    <div>
                        <h3 className={`text-2xl font-bold mb-6 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{subtitle}</h3>
                        <p className={`mb-6 leading-relaxed ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>{description1}</p>
                    ...
                    <div className="space-y-6">
                        <div
                            ...
                            <p className={isDarkMode ? 'text-gray-300' : 'text-gray-600'}>{labels.backendDesc}</p>
                        </div>
                        <div
                            className={`p-6 rounded-lg border ${isDarkMode ? 'bg-gray-700/50 border-gray-600' : 'bg-gray-50 border-gray-200'}`}>
                            <Globe className="text-purple-400 mb-4" size={32}/>
                            <h4 className="text-xl font-semibold mb-2">{labels.frontend}</h4>
                            <p className={isDarkMode ? 'text-gray-300' : 'text-gray-600'}>{labels.frontendDesc}</p>
                        </div>
                        <div
                            className={`p-6 rounded-lg border ${isDarkMode ? 'bg-gray-700/50 border-gray-600' : 'bg-gray-50 border-gray-200'}`}>
                            <Database className="text-emerald-400 mb-4" size={32}/>
                            <h4 className="text-xl font-semibold mb-2">{labels.database}</h4>
                            <p className={isDarkMode ? 'text-gray-300' : 'text-gray-600'}>{labels.databaseDesc}</p>
                        </div>
                    </div>
                </div>
            </div>
